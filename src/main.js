...
class SceneCreator {
    ...
    scale = 1;
    animating = 0;
    constructor(container, scale, camPos, targetPos) {
        ...
        this.cWidth = 0;
        this.cHeight = 0;
        this.camera = new THREE.PerspectiveCamera(50, this.cWidth / this.cHeight, 0.1 * this.scale, 2000 * this.scale);
        if (camPos) {
    ...
    addControls(overrides = {}) {
        ...
        this.controls.target.set(this.initialTargetPos.x, this.initialTargetPos.y, this.initialTargetPos.z);
        return this;
    }
    /**
    ...
    animateModelColor(name, color, duration = 2) {
    ...
    animateModelOpacity(name, value, duration = 2) {
        const obj = this.scene.getObjectByName(name);
        if (obj) {
            obj.traverse((mesh) => {
                if (mesh instanceof THREE.Mesh) {
    ...
    animateModelPosition(name, newPosition, duration = 2) {
        const obj = this.scene.getObjectByName(name);
        if (obj) {
    ...
    renderLoop() {
        ...
        const renderer = this.renderer;
        const camera = this.camera;
        if (camera.position.x !== this.prevCamPos.x ||
    ...
    moveCamera(newPosCam, newPosTarget, callback) {
        const camera = this.camera;
        let reEnable;
        if (this.controls) {
        ...
        gsap.to(camera.position, {
            ...
            onComplete() {
                if (this.controls)
                    this.controls.enabled = reEnable;
